...
<div class="flex items-center justify-center min-h-[calc(100vh-8rem)] px-4 py-8">
    <div class="w-full max-w-lg bg-white rounded-2xl shadow p-8">
        <h2 class="text-2xl font-bold mb-6 text-center">Post a Job</h2>
        ...
        <form method="POST" action="{{ route('jobs.store') }}" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select id="salary_type" name="salary_type" required
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="" disabled {{ old('salary_type') ? '' : 'selected' }}>Select period</option>
                        @foreach (\App\Enums\SalaryType::cases() as $type)
                            <option value="{{ $type->value }}" {{ old('salary_type') === $type->value ? 'selected' : '' }}>
                                {{ $type->label() }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <input id="deadline" name="deadline" type="date" value="{{ old('deadline') }}" required
                    class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                @error('deadline')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
