        ...
        <form method="POST" action="{{ route('profile.store') }}" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input id="grad_year" name="grad_year" type="number" value="{{ old('grad_year') }}" required
                        min="1900" max="2100"
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    @error('grad_year')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea id="about" name="about" rows="4" maxlength="1000"
                    class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none">{{ old('about') }}</textarea>
                @error('about')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
