...
<div class="flex items-center justify-center min-h-[calc(100vh-8rem)] px-4 py-8">
    <div class="w-full max-w-lg bg-white rounded-2xl shadow p-8">
        <h2 class="text-2xl font-bold mb-6 text-center">Edit Experience</h2>

        <form method="POST" action="{{ route('experience.update', $entry) }}" class="space-y-4" id="experience-form">
            @csrf
            @method('PUT')

            <div>
                <label for="title" class="block text-sm font-medium mb-1">Job Title</label>
                <input id="title" name="title" type="text" value="{{ old('title', $entry->title) }}" required
            ...
            <div>
                ...
                <input id="company" name="company" type="text" value="{{ old('company', $entry->company) }}" required
                    class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                @error('company')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="start_month" name="start_month" required
                        ...
                        @for ($m = 1; $m <= 12; $m++)
                            <option value="{{ $m }}" {{ old('start_month', $entry->start_month) == $m ? 'selected' : '' }}>
            ...
            <div>
                <label class="inline-flex items-center gap-2">
                    <input type="checkbox" id="current_role" name="current_role"
                        {{ old('current_role', is_null($entry->end_month) ? true : false) ? 'checked' : '' }}
            ...
            <div class="grid grid-cols-2 gap-4" id="end-date-fields"
                @if(old('current_role', is_null($entry->end_month))) style="display: none;" @endif>
                <div>
                    ...
                    <select id="end_month" name="end_month"
                        ...
                        <option value="">Select</option>
                        @for ($m = 1; $m <= 12; $m++)
                            <option value="{{ $m }}" {{ old('end_month', $entry->end_month) == $m ? 'selected' : '' }}>
                                {{ DateTime::createFromFormat('!m', $m)->format('F') }}
                            </option>
                        @endfor
                    </select>
                ...
                <div>
                    ...
                    <input id="end_year" name="end_year" type="number" value="{{ old('end_year', $entry->end_year) }}"
                        min="1900" max="2100"
                        class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    @error('end_year')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                    class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('description', $entry->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
